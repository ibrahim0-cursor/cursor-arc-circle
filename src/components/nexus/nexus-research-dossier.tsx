...
export function NexusResearchDossierPanel({ token }: { token: TrendingMarketToken }) {
  ...
  return (
    <div className="nexus-research-dossier space-y-3">
      <div className="rounded-2xl border border-violet-400/25 bg-gradient-to-br from-violet-500/12 via-transparent to-cyan-500/5 px-4 py-3">
        <div className="mb-1 flex items-center gap-2">
          <Sparkles className="h-4 w-4 text-violet-200" />
          <p className="text-xs font-bold uppercase tracking-wider text-violet-200/90">At a glance</p>
          {loading && <Loader2 className="h-3.5 w-3.5 animate-spin text-white/50" />}
        </div>
        <p className="text-sm leading-relaxed text-white/90">
          {dossier?.atAGlance ?? (loading ? "Building pro trader dossier…" : error ?? "Select a token")}
        </p>
        {dossier?.dataNotes?.length ? (
          <p className="mt-2 text-[10px] text-white/45">{dossier.dataNotes.join(" · ")}</p>
        ) : null}
      </div>
        ...
        <>
          ...
          <NexusCollapsible
            ...
            hint={
              ...
              `${dossier.creatorRisk.verdict} · Bubblemaps explorer`
            }
            variant="intel"
            icon={Shield}
            defaultOpen={false}
          ...
          <NexusCollapsible
            ...
            hint={`${dossier.pattern.label} · 15m & 1h`}
            variant="technical"
            icon={BarChart3}
            defaultOpen={false}
            showCollapseHint
                ...
                {dossier.technical.map((tf) => (
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-2 text-xs">
                      <span>
                        RSI {tf.rsi14}{" "}
                        <SignalBadge signal={tf.rsiSignal} />
                      </span>
                      <span>
